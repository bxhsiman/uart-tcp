#!/usr/bin/env python3
# tcp_server_framed.py
#
# 支持带长度前缀的LiDAR数据包接收器
# 协议格式：[4字节长度(网络字节序)][N字节数据]
#
# Usage:
#   python get_data_server_framed.py --port 3333 --outfile dump.hex
...
class FramedPacketProcessor:
    ...
    def process_framed_stream(self, conn: socket.socket):
        """处理带长度前缀的数据流"""
        print("开始处理带长度前缀的数据流...")
        ...
        while True:
            try:
                ...
                if packet_length != LIDAR_PACKET_SIZE:
                    self._log_error(f"意外的包长度: {packet_length}, 期望: {LIDAR_PACKET_SIZE}")
                    self.error_count += 1
                    continue
                
                # 读取LiDAR数据包
                packet_data = self._recv_exact(conn, packet_length)
                if not packet_data:
                    break
                    
                # 处理LiDAR包
                self._process_lidar_packet(packet_data)
                
            except Exception as e:
                self._log_error(f"处理数据流时出错: {e}")
                break
                
        print(f"数据流处理结束. 总计: {self.packet_count}包, {self.lost_count}丢失, {self.error_count}错误")
    ...
    def _process_lidar_packet(self, packet: bytes):
        ...
        if current_time - self.last_log_time > 5.0:
            print(f"统计: 收到包={self.packet_count}, 丢失包={self.lost_count}, 错误={self.error_count}")
...
def serve_framed_connection(listen_host: str, port: int, outfile: Path):
    """监听并处理带长度前缀的连接"""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as srv:
        ...
        with conn:
            ...
            processor = FramedPacketProcessor(outfile)
            
            # 写入会话开始标记
            with outfile.open("ab", buffering=0) as f:
                timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
                f.write(f"\n\n=== 带长度前缀会话开始 {timestamp} ===\n".encode())
                f.write("# 协议: [4字节长度][44字节LiDAR数据]\n".encode())
            
            processor.process_framed_stream(conn)
...
def main():
    p = argparse.ArgumentParser(description="TCP server with length-prefixed framing for LiDAR data.")
    ...
    p.add_argument("--outfile", default="dump_framed.hex", help="Destination file path")
    args = p.parse_args()
    ...
    with out_path.open("w") as f:
        f.write("# 激光雷达数据包分析日志 (带长度前缀协议)\n")
        f.write("# 协议格式: [4字节网络字节序长度][44字节LiDAR数据]\n")
    ...
    try:
        while True:
            serve_framed_connection(args.host, args.port, out_path)
