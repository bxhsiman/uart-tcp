...
bool send_buffered_frames(void) {
    ...
    for (int i = 0; i < g_buffered_frames; i++) {
        if (g_frame_buffer[i].valid) {
            ...
            LOG_D(TAG, "📋 发送帧#%d (大小: %d字节)", i+1, LIDAR_FRAME_SIZE);
            ...
            while (sent < LIDAR_FRAME_SIZE) {
                ...
                sent += ret;
                LOG_D(TAG, "📡 已发送 %d/%d 字节", sent, LIDAR_FRAME_SIZE);
            }
            
            if (sent == LIDAR_FRAME_SIZE) {
                successfully_sent++;
                g_total_frames_sent++;
                g_total_bytes_sent += sent;
                LOG_D(TAG, "✅ 帧#%d发送成功 (%d字节)", i+1, sent);
            } else {
                LOG_E(TAG, "❌ 帧#%d发送不完整 (%d/%d字节)", i+1, sent, LIDAR_FRAME_SIZE);
            }
        } else {
